...
import { getProfile } from "@/services/astrologer.service";
...
function AstrologerHeader() {
  ...
  useEffect(() => {
    getProfile()
      .then((response) => {
        setProfile({
          name: response.data.display_name || response.data.full_name || "Astrologer",
  ...
  useEffect(() => {
    api<NotificationPreview[]>("/notifications", {}, "astrologer")
      .then((res) => {
