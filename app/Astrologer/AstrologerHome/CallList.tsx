  ...
  return (
    <div className="w-full rounded-2xl bg-white p-4 shadow-md space-y-4 lg:max-w-[770px] ">
      ...
      <div className="flex justify-between items-center h-[26px] opacity-100">
        ...
        <button className="text-purple-600 text-[12px] font-medium">
          Filter
        </button>
      </div>
      ...
      {data.map((item, index) => (
        ...
        >
          ...
          <div className="flex items-center gap-4">
            {/* Icon */}
            <div className="w-[55px] h-[55px] pt-[12px] pr-[21px] pb-[12px] pl-[21px] rounded-full bg-gradient-to-r from-yellow-100 to-purple-100 flex items-center justify-center">
              {item.type === "call" ? (
                <Phone className="text-purple-600 w-12 h-12" />
              ) : (
                <Video className="text-purple-600 w-12 h-12" />
              )}
            </div>
