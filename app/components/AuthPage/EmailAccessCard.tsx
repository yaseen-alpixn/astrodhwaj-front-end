...
function EmailAccessCard({ role }: EmailAccessCardProps) {
  const userButtonClass =
    role === "user"
      ? "border-[#7a1eb1]/35 bg-[#f7f0ff] text-[#0085FF]"
          ...
          <div className="mx-auto flex max-w-[410px] flex-col">
            ...
            <div className="mt-4">
              ...
              <input
                ...
                className="mt-1.5 h-10 w-full rounded-lg border border-black/18 bg-white px-3.5 text-[0.9rem] text-[#161616] outline-none transition focus:border-[#0085FF] focus:ring-4 focus:ring-[#7a1eb1]/12"
              />
            </div>
            ...
            <Link
              href="/user/home"
              className="mt-3 inline-flex h-10 items-center justify-center rounded-lg bg-[linear-gradient(90deg,#0085FF_0%,#7013b7_100%)] px-6 text-[0.9rem] font-semibold text-white shadow-[0_14px_40px_rgba(122,30,177,0.28)] transition-transform hover:-translate-y-0.5 active:translate-y-0"
            >
              Create Account
            </Link>
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
              <Link
                href="/signup/user"
                className={`inline-flex h-10 items-center justify-center rounded-lg border px-3 text-center text-[0.9rem] font-medium transition-colors hover:border-[#0085FF] hover:text-[#0085FF] ${userButtonClass}`}
              >
                Login as User
              </Link>
              <Link
                href="/Astrologer"
                className={`inline-flex h-10 items-center justify-center rounded-lg border px-3 text-center text-[0.9rem] font-medium transition-colors hover:border-[#0085FF] hover:text-[#0085FF] ${astrologerButtonClass}`}
              >
                Login as Astrologer
              </Link>
            </div>
