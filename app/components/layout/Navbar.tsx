    ...
    >
      <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between px-5 py-3 sm:px-8 lg:px-10">
        ...
        <nav className="hidden lg:flex items-center gap-6 flex-wrap">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium whitespace-nowrap text-[#171717] transition-colors hover:text-[#0085FF]"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden lg:flex items-center gap-3">
          <Link
            href="/donate"
            className="rounded-xl bg-[#4898E1] px-6 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5"
          >
            Donate
          </Link>
          <Link
            href="/login"
            className="rounded-xl bg-gradient-to-l from-[#DD9A29] to-[#0085FF] px-6 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5"
          >
            Login
          </Link>
        </div>
